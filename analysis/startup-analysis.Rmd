...
format_mil <- \(x){format(x/1e6, nsmall=1, big.mark = ",", scientific = FALSE, drop0trailing=T, zero.print=F)}
...
faang <- investors |> 
  filter(name %in% c("Meta", "Apple", "NVIDIA")) |> 
  mutate(criteria = "Other FAANG")
...
faang_osi_ai <- control_investments |>
  filter(criteria == "Other FAANG") |> 
  mutate(cloud_giant = investor_name) |> 
  bind_rows(cloud_investments) |> 
  select(cloud_giant, org_uuid) |> 
  distinct() |> 
  left_join(select(orgs, uuid, name, category_list), by = c("org_uuid" = "uuid"))

faang_osi_ai |> 
  group_by(cloud_giant) |> 
  summarize(
    os = sum(str_detect(category_list, "Open Source"), na.rm=T)/n(),
    ai = sum(str_detect(category_list, "Artificial Intelligence"), na.rm=T)/n()
  ) |> 
...
```{r}
cloud_sectors |> 
  ggplot(aes(x = cloud_giant, y = category_list, fill = n_deals)) + 
  geom_tile() + 
